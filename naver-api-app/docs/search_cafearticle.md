...
네이버의 공개된 카페 게시글들을 검색하여 결과를 반환받을 수 있는 비로그인 방식의 오픈 API입니다.

## 1. 개요 및 한도
* **설명**: 네이버 카페의 공개 게시판 글을 대상으로 검색을 실행하여 결과를 JSON 또는 XML 형식으로 반환합니다.
* **하루 호출 한도**: 검색 API 전체 통합하여 하루 총 25,000회 제한
* **방식**: 비로그인 방식 (HTTP Header에 Client ID, Secret 전송)

## 2. API 스펙
* **요청 URL**: 
  * JSON 포맷: `https://openapi.naver.com/v1/search/cafearticle.json`
  * XML 포맷: `https://openapi.naver.com/v1/search/cafearticle.xml`
* **프로토콜**: HTTPS
* **HTTP Method**: GET
...
* **주요 응답 필드**:
  ...
  * `items`: 개별 검색 결과 기사 배열
    * `title`: 카페 게시글 제목 (검색 키워드와 매칭된 단어는 `<b>` 태그로 감싸져 제공됨)
    * `link`: 카페 게시글 상세 URL
    * `description`: 카페 게시글 본문 요약 (검색 키워드와 매칭된 단어는 `<b>` 태그 포함)
    * `cafename`: 게시글이 속한 카페 이름
    * `cafeurl`: 게시글이 속한 카페 홈 URL
...
## 4. 요청 예시 (cURL)
```bash
curl "https://openapi.naver.com/v1/search/cafearticle.json?query=%EC%A3%BC%EC%8B%9D&display=10&start=1&sort=sim" \
