...
네이버 뉴스의 기사들을 검색하여 결과를 반환받을 수 있는 비로그인 방식의 오픈 API입니다.

## 1. 개요 및 한도
* **설명**: 네이버 뉴스 검색 결과를 JSON 또는 XML 형식으로 반환합니다.
* **하루 호출 한도**: 검색 API 전체 통합하여 하루 총 25,000회 제한
* **방식**: 비로그인 방식 (HTTP Header에 Client ID, Secret 전송)

## 2. API 스펙
* **요청 URL**: 
  * JSON 포맷: `https://openapi.naver.com/v1/search/news.json`
  * XML 포맷: `https://openapi.naver.com/v1/search/news.xml`
* **프로토콜**: HTTPS
* **HTTP Method**: GET
...
* **주요 응답 필드**:
  ...
  * `items`: 개별 검색 결과 기사 배열
    * `title`: 뉴스 기사의 제목 (검색 키워드와 매칭된 단어는 `<b>` 태그로 감싸져 제공됨)
    * `originallink`: 뉴스 기사 원문 제공 언론사 URL
    * `link`: 네이버 뉴스 제공 상세 URL (네이버 미제공 시 원문 URL 반환)
    * `description`: 뉴스 기사 본문 요약 (검색 키워드와 매칭된 단어는 `<b>` 태그 포함)
    * `pubDate`: 뉴스 기사가 네이버에 등록되거나 발행된 시간 (RFC 822 형식)
